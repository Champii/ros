...
impl BootInfoFrameAllocator {
    pub unsafe fn init(multiboot_information_address: usize) -> Self {
        Self {
            // boot_info: AvailableFrames::new(multiboot_information_address),
            multiboot_information_address,
            next: 0,
        }
    }

    pub fn available_frames(&self) -> impl Iterator<Item = UnusedPhysFrame> {
        // let boot_info: BootInformation = unsafe { multiboot2::load(multiboot_information_address) };

        let boot_info: &'static BootInformation =
            unsafe { load(self.multiboot_information_address) };

        let regions = boot_info.memory_map_tag().unwrap().memory_areas();

        let addr_ranges = regions.skip(1).map(|r| r.start_address()..r.end_address());

        let frame_addresses = addr_ranges.flat_map(|r| r.step_by(4096)).filter(|r| {
            let boot_info2: &'static BootInformation =
                unsafe { load(self.multiboot_information_address) };

            let elf_sections_tag = boot_info2
                .elf_sections_tag()
                .expect("Elf-sections tag required");
            ...
            let multiboot_start = boot_info2.start_address() as u64;
            let multiboot_end = multiboot_start + (boot_info2.total_size() as u64);

            let kernal_range = kernel_start..kernel_end;
            let multiboot_range = multiboot_start..multiboot_end;

            !kernal_range.contains(&r) && !multiboot_range.contains(&r)
        });
